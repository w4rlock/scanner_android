	...
	public void onCreate(SQLiteDatabase database) {
		try {
			database.execSQL("CREATE TABLE IF NOT EXISTS " + DBUtil.TBL_INV
					+ "( " + DBUtil.TINV_PLU + " VARCHAR(20),"
					+ DBUtil.TINV_TCOD + " VARCHAR(20)," + DBUtil.TINV_LOC
					+ " VARCHAR(20)," + DBUtil.TINV_CANT + " DECIMAL(10,3));");

			database.execSQL("CREATE TABLE IF NOT EXISTS " + DBUtil.TBL_SER
					+ " ( " + DBUtil.TSER_CODLOC + " VARCHAR(20), "
					+ DBUtil.TSER_CODPLU + " VARCHAR(20), "
					+ DBUtil.TSER_SERIAL + " VARCHAR(100)" + ");");
